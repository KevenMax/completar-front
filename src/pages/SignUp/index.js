...
class SignUp extends Component{
  ...
  handleSignUp = async e => {
    ...
    if(!email || !password || !password_confirmation){
      toast.error('Preencha e-mail, senha e confirmação de senha para continuar!', {position: toast.POSITION.TOP_RIGHT});
    }else{
      try{
        await api.post("/auth", { email, password, password_confirmation });
        const authenticate =  await api.post('/auth/sign_in', {email, password});
        const headers = authenticate.headers;
        setTokens(headers['access-token'], headers['uid'], headers['expiry'], headers['client'], headers['token-type']);
      ...
      <section id="sign-up">
        <div className="container">
          ...
          <form onSubmit={this.handleSignUp}>
            ...
            <input type="password" placeholder="Confirmação de senha" onChange={e => this.setState({password_confirmation: e.target.value}) }/>
